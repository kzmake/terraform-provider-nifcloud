...
func logRequest(r *request.Request) {
	logBody := r.Config.LogLevel.Matches(nifcloud.LogDebugWithHTTPBody)
	bodySeekable := nifcloud.IsReaderSeekable(r.Body)
	...
	if logBody {
		if !bodySeekable {
			r.SetReaderBody(nifcloud.ReadSeekCloser(r.HTTPRequest.Body))
		}
...
func logResponse(r *request.Request) {
	...
	handlerFn := func(req *request.Request) {
		...
		if req.Config.LogLevel.Matches(nifcloud.LogDebugWithHTTPBody) {
