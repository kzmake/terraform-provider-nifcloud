...
const (
	...
	// API request that was canceled. Requests given a nifcloud.Context may
...
func New(cfg nifcloud.Config, clientInfo metadata.ClientInfo, handlers Handlers,
...
//
//     svc.PutObjectWithContext(ctx, params, request.WithLogLevel(nifcloud.LogDebugWithHTTPBody)
func WithLogLevel(l nifcloud.LogLevelType) Option {
	return func(r *Request) {
		r.Config.LogLevel = nifcloud.LogLevel(l)
	}
}
...
// context nifcloud.BackgroundContext will be returned.
func (r *Request) Context() nifcloud.Context {
	if r.context != nil {
		return r.context
	}
	return nifcloud.BackgroundContext()
}
...
func (r *Request) SetContext(ctx nifcloud.Context) {
	if ctx == nil {
...
func (r *Request) WillRetry() bool {
	if !nifcloud.IsReaderSeekable(r.Body) && r.HTTPRequest.Body != NoBody {
		return false
	}
	return r.Error != nil && nifcloud.BoolValue(r.Retryable) && r.RetryCount < r.MaxRetries()
}
...
func debugLogReqError(r *Request, stage string, retrying bool, err error) {
	if !r.Config.LogLevel.Matches(nifcloud.LogDebugWithRequestErrors) {
		return
	}
...
func (r *Request) getNextRequestBody() (io.ReadCloser, error) {
	...
	l, err := nifcloud.SeekerLen(r.Body)
	if err != nil {
	...
	if l == 0 {
		...
		//
		// This would only happen if a nifcloud.ReaderSeekerCloser was used with
...
func (r *Request) Send() error {
	...
	for {
		if nifcloud.BoolValue(r.Retryable) {
			if r.Config.LogLevel.Matches(nifcloud.LogDebugWithRequestRetries) {
