...
func WithWaiterLogger(logger nifcloud.Logger) WaiterOption {
	return func(w *Waiter) {
		w.Logger = logger
	}
}
...
type Waiter struct {
	...
	SleepWithContext func(nifcloud.Context, time.Duration) error
}
...
// Use nifcloud.BackgroundContext if no context is available.
//
...
func (w Waiter) WaitWithContext(ctx nifcloud.Context) error {
	...
	for attempt := 1; ; attempt++ {
		...
		if sleepFn := req.Config.SleepDelay; sleepFn != nil {
			...
			if sleepCtxFn == nil {
				sleepCtxFn = nifcloud.SleepWithContext
			}
...
func (a *WaiterAcceptor) match(name string, l nifcloud.Logger, req *Request, err error) (bool, error) {
	result := false
	var vals []interface{}
...
func waiterLogf(logger nifcloud.Logger, msg string, args ...interface{}) {
	if logger != nil {
